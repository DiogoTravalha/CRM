...
const FabricarTagPage = () => {
  ...
  const { SearchProductionTag, digitado, setDigitado, productionTag } =
    useAuth();
  ...
  async function AddCliente() {
    const fabricartag_name = nome === undefined ? "" : nome;
    const fabricartag_valor = valor;
    ...
    try {
      const response = await fetch(
        "https://api.devteam.vps-kinghost.net/service/production/tag/create",
        {
          method: "POST",
          headers: {
  ...
  async function delCliente(item) {
    try {
      const deleteTodo = await fetch(
        `https://api.devteam.vps-kinghost.net/service/production/tag/delete/${item.id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  async function editCliente() {
    ...
    try {
      const response = await fetch(
        `https://api.devteam.vps-kinghost.net/service/production/tag/update/${id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  function pageLimits() {
    const pagination = productionTag.slice(
    ...
    const pageNumbers = [];
    for (let i = 1; i <= Math.ceil(productionTag.length / todosPerpage); i++) {
      pageNumbers.push(i);
          ...
          <div className="col">
            <Card className="shadow">
              <CardHeader className="border-0">
                <h3 className="mb-0">Lista Tag de Fabricação</h3>
              </CardHeader>
...
export default FabricarTagPage;
