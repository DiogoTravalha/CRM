...
const CalendarioTagPage = () => {
  ...
  async function AddCliente() {
    const calendariotag_name = nome === undefined ? "" : nome;
    const calendariotag_valor = valor;
    ...
    try {
      const response = await fetch(
        "https://api.devteam.vps-kinghost.net/schedule/tag/create",
        {
          method: "POST",
          headers: {
  ...
  async function delCliente(item) {
    try {
      const deleteTodo = await fetch(
        `https://api.devteam.vps-kinghost.net/schedule/tag/delete/${item.id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  async function editCliente() {
    ...
    try {
      const response = await fetch(
        `https://api.devteam.vps-kinghost.net/schedule/tag/update/${id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  function pageLimits() {
    const pagination = scheduleTag.slice(
    ...
    const pageNumbers = [];
    for (let i = 1; i <= Math.ceil(scheduleTag.length / todosPerpage); i++) {
      pageNumbers.push(i);
          ...
          <div className="col">
            <Card className="shadow">
              <CardHeader className="border-0">
                <h3 className="mb-0">Lista Tag do Calendário</h3>
              </CardHeader>
...
export default CalendarioTagPage;
